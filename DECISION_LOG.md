...
## 2026-07-20 — Design system refresh: data-ops console direction (amends ADR-0014)

**Context:** After watching the live app in a browser for the first time, the user asked for a more "business-appealing" visual direction referencing Palantir-style enterprise data platforms — explicitly not a copy, take the genre and make it distinct — then gave two rounds of concrete feedback live: the incident sidebar had too little contrast between rows and background, and afterward that the overall palette was still too dark/small text required too much focus to read.

**Decision:** Three passes, each verified live in the browser (not just built/tested — this is subjective visual work no automated test can validate):

1. **Direction pass:** swapped the generic sky-blue accent (`#38bdf8`) for a precise teal (`#2fb8c4`, a hue distinct from every severity color so it never gets confused with status), sharpened corners across all primitives (`rounded-md`/`rounded-full` → `rounded-sm`), added real webfonts via `next/font/google` (Inter for body, JetBrains Mono for technical content — badges, labels, a new `.text-kicker` utility), and a very restrained background texture for depth.
2. **Contrast fix (sidebar):** incident list rows had only a colored left accent border and relied on a barely-perceptible `bg-card` vs `bg-background` difference for the rest of their shape — added a full border outline per row and a clearer selected/hover state (teal border + ring), on top of a `--color-card` bump.
3. **Legibility fix (whole app):** replaced the near-black palette with a real elevation ladder modeled on how GitHub/Linear/Vercel actually do dark mode — none of them go near-black; background lifted `#0a0b0d` → `#101114`, card `#191b1f` → `#1c1e23`, border `#26272b` → `#34363c`, muted-foreground (used constantly for labels/descriptions) brightened `#9a9ba3` → `#b4b5bd`, foreground `#e7e7ea` → `#f2f2f4`. Badge text bumped 10px → 11px with tracking pulled back (`tracking-wider` → `tracking-wide`) — tiny uppercase tracked mono text was the single biggest legibility offender. Removed the background grid-line texture entirely (kept only a faint top vignette) since it competed with text instead of adding perceptible depth.

**Rationale:** Subjective design feedback from actually looking at the running app outweighs any a priori color-theory judgment — every fix here came from a specific, concrete complaint, not a redesign-for-its-own-sake. The elevation-ladder principle (never near-black, each surface step clearly lighter than the one below) is the actual mechanism the "top notch platform" reference platforms use, not a stylistic guess.
...
## 2026-07-20 — Live progress: streaming AI drafts and background incident refresh (ADR-0020)

**Context:** Third and final item from the earlier product critique — "we see no live progress." Full rationale in [ADR-0020](docs/adr/0020-live-progress-streaming-draft-and-polling.md).

**Decision:** `POST /incidents/:id/analyze/draft/stream` (new, Server-Sent Events) streams the model's real text output token-by-token via the Anthropic SDK's streaming API, then validates the complete text through the exact same pipeline `analyze/draft` already used and emits a final `result` event. `apps/web` consumes it with a hand-rolled SSE parser (`fetch` + manual reader, since `EventSource` can't do POST + auth headers) and shows the live text in a "drafting…" preview before the form populates. Separately, the Command Center now polls the selected incident's summary/timeline/analyses every 8 seconds in the background, with a new `LiveSyncIndicator` ("Synced Xs ago") showing it's actually happening.

**Two real bugs found and fixed via live testing, not assumed away:**

1. The global `TenantRlsInterceptor` uses `lastValueFrom(next.handle())`, which collapses a multi-emission Observable to only its last value — for the new SSE route this would have silently swallowed every chunk and delivered only the final event, defeating streaming entirely with no error raised. Fixed with a new `@SkipTenantRls()` decorator (Reflector-checked, mirrors the existing `@Roles()` pattern) exempting the streaming route from that global transaction; `AiDraftService` establishes its own RLS context narrowly around just its two DB reads instead of holding one open for the whole stream.
2. Nest's SSE machinery commits response headers on a 0ms timer regardless of whether the request is about to fail, unless the failure happens with zero I/O before it. Confirmed live, repeatedly: the `available` check (no I/O) reliably wins that race and returns a clean 503; a `NotFoundException` from the incident DB lookup (real I/O) always loses it and ends up reported as an in-stream `error` event instead of a 404. Both delivery paths already work identically from the frontend's perspective, but the doc comments and e2e test now say so precisely rather than implying a broader guarantee than what's actually true.

**Status:** Done. Verified live against the real Anthropic API: watched real incremental SSE chunks arrive (71 events for one real draft), confirmed the final `result` event resolves into the same validated draft the non-streaming endpoint produces, confirmed the `available`-unavailable path returns a real 503 both via a fresh e2e run (no `.env` present) and confirmed the incident-not-found path reports via an `error` event (only testable live, with a real key — the availability check always short-circuits first without one, so this path is unreachable in CI by construction, disclosed rather than silently skipped). New unit tests: `AnthropicLlmClient` streaming path is exercised through `AiDraftService`'s split (`prepareDraftPrompt`/`streamDraftText`), `TenantRlsInterceptor`'s skip behavior (4 new tests, including one proving it no longer collapses a multi-emission Observable), `LiveSyncIndicator` (2 tests, real fake-timer-driven ticking). `IntelligenceAnalysisForm`'s AI-drafting tests rewritten around a mocked `streamSse()`, including a properly-gated test proving the live text genuinely appears before the result resolves (not a race-prone assertion against an instantly-resolving mock). 224 `apps/api` unit tests + 9 e2e suites (15 tests) + 56 `apps/web` unit tests, `lint`/`build`/`format` all clean.

---

## 2026-07-20 — Entity-relationship intelligence graph + analyst-activity audit log (ADR-0021)

**Context:** The user set new direction for the project's mission: government intelligence analysis — data integration, link analysis, geospatial analysis, secure collaboration, in the spirit of Palantir Gotham's actual capabilities but explicitly the opposite of its opacity reputation, with the core mission being faster/smarter analysis while protecting civil liberties via human-analyst control, not automated surveillance. Asked to build the audit log next. Full rationale in [ADR-0021](docs/adr/0021-entity-relationship-graph-and-audit-log.md).

**Decision:** Six new tenant-scoped tables: `Entity` (person/organization/location/event) and `Relationship` (typed, directed edges) form the graph; `EntityEvidenceLink`/`RelationshipEvidenceLink` require every entity and relationship to cite the real evidence it came from — never a bare assertion, same rule the confidence breakdowns (ADR-0019) already follow. Relationships always start `SUGGESTED`; confirming requires a real state transition plus at least one evidence citation, checked in code. `EntityMergeSuggestion` handles "is this the same person?" the same way — suggested, never auto-applied. `AuditLogEntry` is a new append-only table logging every read/write against the graph — who, what, when, and (for search/view actions specifically) why, with `reason` enforced as a required field at the DTO layer, not optional. `GET /audit-log` is ADMIN-gated, since the point of an audit log is that the people being audited don't control who reviews it.

**Rationale:** Palantir's real-world criticism was never really about the technology finding connections — it's the opacity of not knowing who looked up what, or why. Logging analyst activity with enforced purpose statements, reviewable by a role the analyst doesn't control, is the concrete mechanism that makes "protect civil liberties" real instead of aspirational. No frontend exists for any of this yet — this pass is the data model and API layer, matching the explicit "build the audit log next" scope; `packages/shared` types document the wire contract ahead of the UI, same pattern ADR-0019 followed.

**Status:** Done. 16 new unit tests (`AuditLogService`, `EntitiesService`, `RelationshipsService` — covering evidence-citation requirements, the SUGGESTED→CONFIRMED/REJECTED state machine, and every audit-log call site). New e2e spec (`entities-and-audit-log.e2e-spec.ts`, 3 tests) against a real Postgres: builds a real entity+relationship graph from real evidence, confirms `reason` is enforced (400 without it), confirms the full audit trail is created correctly, proves real cross-tenant isolation (404/empty-results/audit-log isolation), and proves the ADMIN-only gate on the audit log review endpoint (403 for a MEMBER). 240 `apps/api` unit tests + 10 e2e suites (18 tests), `lint`/`build`/`format` all clean.

---

## 2026-07-20 — Geospatial entity search: mapping where things happened (ADR-0022)

**Context:** Third of the four intelligence-mission pillars named alongside link analysis (ADR-0021) — "geospatial analysis, mapping where things happened." Full rationale in [ADR-0022](docs/adr/0022-geospatial-entity-search.md).

**Decision:** `Entity` gains real `latitude`/`longitude` columns (not buried in the existing free-form `attributes` JSON) — both or neither, range-validated, enforced in `CreateEntityDto` via `@ValidateIf`. Two new endpoints: `GET /entities/nearby` (radius search from a point, sorted nearest-first, real computed `distanceKm`) and `GET /entities/map` (every located entity, no radius, for a future map UI). Both logged to the audit log (ADR-0021) with the same enforced-`reason` purpose-limitation rule every other entity read follows. Distance is computed via the Haversine formula in application code, not PostGIS — a real Postgres extension/Docker-image change for a capability this stage doesn't need yet; the honest scope note is that this doesn't have a spatial index and would need to migrate to PostGIS if a tenant's located-entity count ever got large.

**Rationale:** Fetching a tenant's located entities via a normal Prisma query and filtering/sorting distance in JavaScript stays entirely within the existing RLS-transparent tenant-aware proxy — no raw SQL to separately audit for tenant-isolation correctness, and no new infrastructure. `haversineDistanceKm()` is verified against real, well-known distances between real cities, not synthetic test coordinates.

**Status:** Done, verified live end-to-end against real Postgres with real coordinates: created three real `LOCATION` entities (Washington D.C., New York, Los Angeles offices) citing real evidence, confirmed a latitude-with-no-longitude submission is rejected (400) before ever reaching the database, confirmed a 500km radius search from D.C. correctly includes D.C./New York (~330km apart) and excludes Los Angeles (~3900km away), confirmed nearest-first ordering, confirmed the map endpoint returns every located entity with no radius filter, and confirmed every one of those reads landed on the audit log. 7 new unit tests (4 for the Haversine formula against known real-world distances, 3 for `EntitiesService.searchNearby`/`getMap`) + 1 new e2e test. 247 `apps/api` unit tests + 11 e2e suites (19 tests), `lint`/`build`/`format` clean.
