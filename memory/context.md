...
- No `apps/web` UI exists yet for Integrations management (Phase 6) — only backend + tests. Decision Intelligence Engine (Phase 4) and Reporting (Phase 5) UI are now done (see above); those, the Command Center + Decision Log (ADR-0014), login, and the `/simulation` facilitator panel (ADR-0013) are the only frontend surfaces built so far.
